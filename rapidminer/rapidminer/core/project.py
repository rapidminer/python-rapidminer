...
class Project():
    ...
    # constant value is coming from
    # com.rapidminer.storage.hdf5.ExampleSetHdf5Writer.java#writeDateData/writeTimeData
    # as of Long.MAX_VALUE/TimeColumn.MISSING_VALUE
    __RM_MISSING_DATETIME_OR_TIME = 9223372036854775807
    ...
    def __init__(self, path="."):
        ...
        if not os.path.exists(path):
            ...
            raise ProjectException(
                "Project '%s' does not exist %s. Please make sure you have a local copy of the project." %
                (os.path.basename(path), msg_dir_part))
...
#####################
    ...
    __from_ts_nanos = np.vectorize(lambda x: datetime.utcfromtimestamp(
        x / 1e9) if x != Project.__RM_MISSING_DATETIME_OR_TIME else None)
    __from_ts_seconds_and_nanos = np.vectorize(lambda x, y: datetime.utcfromtimestamp(
        x + y / 1e9) if x != Project.__RM_MISSING_DATETIME_OR_TIME else None)
    ...
    def __get_data(f, x):
        ...
        return pd.Categorical.from_codes(
            g(r[()]), [decode(s) if not isinstance(s, str) else s for s in mapping], ordered=False)
    ...
    def __get_role(hdf_column):
        if "role" in hdf_column.attrs:
            ...
            if r in Project._METADATA_ROLES:
                ...
                raise ProjectException("Role '" + r + "' not recognized. (Column '" +
                                       decode(hdf_column.attrs.get('name')) + "'.)")
        return None
    ...
    def __create_dataset(f, column, desired_type, index):
        ...
        if desired_type in ["NOMINAL", "BINOMINAL", "POLYNOMINAL", "TEXT", "FILE_PATH"]:
            ...
            if desired_type == "BINOMINAL" and len(mappingvals) > 3:
                raise TooManyBinomialValuesError(
                    "Column '%s' marked as binomial column in rm_metadata attribute, but has more there is more then two distinct values present.", )
            if len(mappingvals) <= 3:
                ...
                try:
                    ...
                    # fixes error: TypeError: Can't implicitly convert non-string objects to
                    # strings (caused by some Python environments)
            ...
            if desired_type == "TIME":
                ...
                seconds = nanoseconds.apply(lambda x: x // int(1e9) if x !=
                                            Project.__PANDAS_MISSING_DATE else Project.__RM_MISSING_DATETIME_OR_TIME)
    ...
    def __write_column(f, frame, column, name, index):
        ...
        try:
            ...
            raise ValueConversionError(
                "Cannot write output file with the desired format. Please review rm_metadata of the pandas DataFrame. Cause: " + str(e))
    ...
    def __write_data_safe(data, filename):
        ...
        if not os.path.exists(os.path.dirname(filename)):
            raise ProjectException(
                "Cannot write file. Parent directory '%s' does not exists." %
